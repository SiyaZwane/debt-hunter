...
/**
 * AC-52: with {@code --project} configured for two monorepo sub-projects, each project's findings
 * are sliced into their own SARIF run, gated as if it were its own scan.
 */
@Tag("integration")
class AC52_MultiProjectSlicingTest {
  ...
  @Test
  void ac52_findingsAreSlicedIntoAPerProjectSarifRun(@TempDir Path outputDir) throws Exception {
    fixture =
        FixtureRepoBuilder.init()
            .commitFile("frontend/App.java", "class App {}", "add App")
            .commitFile("backend/Server.java", "class Server {}", "add Server");

    Finding frontendFinding = finding("f-frontend", "frontend/App.java");
    Finding backendFinding = finding("f-backend", "backend/Server.java");
    ...
    when(engine.analyse(any(), any()))
        .thenReturn(EngineResult.ok(List.of(frontendFinding, backendFinding), List.of(), 5));

    Map<String, String> projects = new LinkedHashMap<>();
    projects.put("frontend", "frontend");
    projects.put("backend", "backend");
    ...
    assertThat(runsByAutomationId).containsKeys("debt-hunter/frontend/", "debt-hunter/backend/");
    assertThat(runsByAutomationId.get("debt-hunter/frontend/").get("results")).hasSize(1);
    assertThat(runsByAutomationId.get("debt-hunter/backend/").get("results")).hasSize(1);

    JsonNode json = mapper.readTree(outputDir.resolve(JsonReporter.FILE_NAME).toFile());
    assertThat(json.get("findings")).hasSize(2);
  }
